<template>
  <div>
    <div v-if="showMethods">
      <h1 class="text-5xl font-semibold mb-10" style="font-family: 'Quantico'">
        ELIGE TU MÉTODO DE {{ name }}
      </h1>
      <div
        class="grid gap-1 grid-cols-1 my-6 mx-auto items-center"
        v-bind:class="{
          'xl:grid-cols-1': paymentMethods.length === 1,
          'xl:grid-cols-2': paymentMethods.length !== 1,
        }"
      >
        <OrderCreditCard
          class="mx-auto text-xl my-5"
    ...
    <div v-else>
      <div>
        <form class="mt-5">
          <h1
            class="text-4xl font-semibold mb-10"
            style="font-family: 'Quantico'"
          >
...
export default {
  ...
  computed: {
    ...
    numberErrors() {
      ...
      !this.$v.newPayment.number.validNumber && errors.push('Formato inválido');
      return errors;
    },
    cvvErrors() {
      const errors = [];
      ...
      !this.$v.newPayment.cvv.validCvv && errors.push('Formato inválido');
      return errors;
    },
    monthErrors() {
      const errors = [];
    ...
    yearErrors() {
      ...
      !this.$v.newPayment.expYear.validYear && errors.push('Formato inválido');
  ...
  methods: {
    ...
    addPaymentMethod() {
      ...
      if (!this.$v.$invalid) {
        axios({
          url: 'http://localhost:8888/payMethod/add',
